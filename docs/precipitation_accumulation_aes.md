...
Parameters are set in `config/precipitation_accumulation_aes.yaml`

Call as
```bash
/path/to/precipitation_accumulation_aes.py
```
...
Using `no-land-mask: true` to mask precipitation over ocean cells is encouraged to save disk space.
...
- `interval 300, floor 0.0,  no_land_mask true, floor_to_zero true` uses 53M of disk space,
- `interval 300, floor 0.01, no_land_mask true, floor_to_zero true` uses 8M of disk space,
- `interval 300, floor 0.01, no_land_mask true,` uses 7.3M of disk space,
- `interval 300, floor 0.1,  no_land_mask true,` uses 3.4M of disk space.
...
- `interval 300, floor 0.0001, no_land_mask true` uses 17M of disk space,
- `interval 300, floor 0.0001, no_land_mask true, lon_min -150 lon_max 50 lat_min 30 lat_max 70` uses 4.5M of disk space,
